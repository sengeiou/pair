...
function MyNnd_ModifyPage()
{
	...
	for (var i = 0; i < topics.length; i++)
	{
		...
		var ans_analy = MyNnd_GetChild(question_list, "DIV", 3);
...
function MyNnd_CatchProblems()
{
	...
	for (var i = 0; i < topics.length; i++)
	{
		...
		var question_list = MyNnd_GetChild(topic, "DIV", 0);
		var question_content = MyNnd_GetChild(question_list, "DIV", 1);
		var operate_area = MyNnd_GetChild(question_list, "DIV", 2);
		var ans_analy = MyNnd_GetChild(question_list, "DIV", 3);
		...
		if (phase_subject != null)
		{
			...
			if (leftTree != null)
			{
				...
				if (selectorPanel != null)
				{
					...
					var dl_diff = MyNnd_GetChild(selectorPanel, "DL", 1);
					...
					if (a_diff != null)
					{
						...
						{
							var filter_item_content = MyNnd_Find_Class(selectorPanel, "filter-item-content");
...
function MyNnd_GetGroup()
{
	...
	try
	{
		...
		{
			/* MyNnd_Log("find selectorPanel", null); */
			var dl_style = MyNnd_GetChild(selectorPanel, "DL", 0);
			var dl_diff = MyNnd_GetChild(selectorPanel, "DL", 1);
			...
			for (var i = 0; i < as_area.length; i++)
			{
				...
				if (a_area.innerHTML.trim() != "全国" && a_area.innerHTML.trim() != "本省")
				{
