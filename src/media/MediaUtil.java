...
public class MediaUtil
{
	 ...
	 * 视频帧抽取的默认时长，单位：s（秒）
	 */
	private static int DEFAULT_TIME_LENGTH = 5;
	/**
	...
	public static boolean setFFmpegPath(String ffmpeg_path)
	{
		...
		{
			LOG.e("--- 设置ffmpeg执行路径失败，因为传入的ffmpeg可执行程序路径为空！ ---");
			return false;
		}
		File ffmpegFile = new File(ffmpeg_path);
		if (!ffmpegFile.exists())
		{
			LOG.e("--- 设置ffmpeg执行路径失败，因为传入的ffmpeg可执行程序路径下的ffmpeg文件不存在！ ---");
			return false;
		}
		FFMPEG_PATH = ffmpeg_path;
//		LOG.v("--- 设置ffmpeg执行路径成功 --- 当前ffmpeg可执行程序路径为： " + ffmpeg_path);
		return true;
	}
	...
	public static boolean isExecutable()
	{
		...
		if (!ffmpegFile.exists())
		{
			LOG.e("--- 工作状态异常，因为传入的ffmpeg可执行程序路径下的ffmpeg文件不存在！ ---");
			return false;
		}
...
//			LOG.v("--- 开始执行FFmpeg指令：--- 执行线程名：" + builder.toString());
			...
			PrintStream errorStream = new PrintStream(ffmpeg.getErrorStream(), true);
			PrintStream inputStream = new PrintStream(ffmpeg.getInputStream(), false);
			errorStream.start();
			inputStream.start();
			// 等待ffmpeg命令执行完
			ffmpeg.waitFor();
...
//			LOG.v("--- 已执行的FFmepg命令： ---" + cmdStr + " 已执行完毕,执行结果： " + resultStr);
			return result;
		}
		catch (Exception e)
		{
			LOG.e("--- FFmpeg命令执行出错！ --- 出错信息： " + e.getMessage());
			return null;
		}
		finally
		{
	...
	public static void convertVideo(File fileInput, File fileOutPut, boolean withAudio, Integer crf, String preset, Integer width, Integer height)
	{
		...
		commond.add("-max_muxing_queue_size");
		commond.add("1024");
		if (!withAudio)
		{
			// 设置是否保留音频
	...
	public static void cutVideoFrame(File videoFile, File fileOutPut, int time)
	{
		cutVideoFrame(videoFile, fileOutPut, time, 0);
	}
	 ...
	 */
	public static void cutVideoFrame(File videoFile, File fileOutPut, int time, int width)
	{
		...
		if (null == info)
		{
			LOG.e("--- 未能解析源视频信息，视频帧抽取操作失败 --- 源视频： " + videoFile);
			return;
		}
	 ...
	 */
	public static void cutVideoFrame(File videoFile, File fileOutPut, int time, int width, int height)
	{
		if (null == videoFile || !videoFile.exists())
		{
			throw new RuntimeException("源视频文件不存在，请检查源视频路径");
		...
		else
		{
			...
			{
				LOG.e("--- 截取视频帧操作出错 --- 错误信息：" + e.getMessage());
			}
			finally
			{
				// 删除用于生成gif的临时文件
	 ...
	 */
	private static void cutVideoFrame(File videoFile, String path, int time, int width, int height, int timeLength, boolean isContinuty)
	{
		...
		if (time > info.getDuration() / 1000 - timeLength)
		{
			time = (int) (info.getDuration() / 1000 - timeLength);
		}
		if (width == 0 || height == 0)
		{
			width = info.getWidth();
		...
		{
			LOG.e("--- 视频帧抽取过程出错 --- 错误信息： " + e.getMessage());
		}
	}
	...
	public static void getAudioFromVideo(File videoFile, File audioFile)
	{
		...
		{
			LOG.e("--- 抽取视频中的音频信息的过程出错 --- 错误信息： " + e.getMessage());
		}
	}

	public static VideoMetaInfo getVideoMetaInfo(File videoFile)
	{
//		VideoMetaInfo videoMetaInfoOld = getVideoMetaInfo(videoFile, false);
		VideoMetaInfo videoMetaInfoNew = getVideoMetaInfo(videoFile, true);
//		LOG.v("old : " + videoMetaInfoOld);
//		LOG.v("new : " + videoMetaInfoNew);
//		if (!videoMetaInfoOld.toString().equals(videoMetaInfoNew.toString()))
//			LOG.e("not equals");
		return videoMetaInfoNew;
	}

	/**
	 * 解析视频的基本信息（从文件中）
	 *
	 * 解析出的视频信息一般为以下格式： Input #0, mov,mp4,m4a,3gp,3g2,mj2, from '6.mp4': Duration: 00:00:30.04, start: 0.000000, bitrate: 19031 kb/s Stream #0:0(eng): Video: h264 (Main) (avc1 / 0x31637661), yuv420p(tv, bt709), 1920x1080, 18684 kb/s, 25 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream #0:1(eng): Audio: aac (LC) (mp4a / 0x6134706D), 48000 Hz, stereo, fltp, 317 kb/s (default)
	 *
	 * 注解： Duration: 00:00:30.04【视频时长】, start: 0.000000【视频开始时间】, bitrate: 19031 kb/s【视频比特率/码率】 Stream #0:0(eng): Video: h264【视频编码格式】 (Main) (avc1 / 0x31637661), yuv420p(tv, bt709), 1920x1080【视频分辨率，宽x高】, 18684【视频比特率】 kb/s, 25【视频帧率】 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream #0:1(eng): Audio: aac【音频格式】 (LC) (mp4a / 0x6134706D), 48000【音频采样率】 Hz, stereo, fltp, 317【音频码率】 kb/s (default)
	 *
	 * @param videoFile 源视频路径
	 * @return 视频的基本信息，解码失败时返回null
	 */
	public static VideoMetaInfo getVideoMetaInfo(File videoFile, boolean isNew)
	{
		if (null == videoFile || !videoFile.exists())
		{
			LOG.e("--- 解析视频信息失败，因为要解析的源视频文件不存在 ---");
			return null;
		}
...
//					LOG.v(line);
					...
					{
//						LOG.v(line);
						line = line.substring("Duration:".length()).trim();
						String[] parts = line.split(",");
						String[] timeParts = parts[0].trim().split("\\.");
						String[] times = timeParts[0].trim().split(":");
						long hours = (long) Integer.parseInt(times[0].trim());
						long minutes = (long) Integer.parseInt(times[1].trim());
						long seconds = (long) Integer.parseInt(times[2].trim());
						long dec = (long) Integer.parseInt(timeParts[1].trim());
						duration = dec + seconds * 1000L + minutes * 60L * 1000L + hours * 60L * 60L * 1000L;
						videoBitrate = Integer.parseInt(parts[2].trim().substring("bitrate:".length()).trim().split(" ")[0].trim());
					}
					else if (line.startsWith("Stream"))
					{
//						LOG.v(line);
						line = line.substring("Stream #0:0(und):".length()).trim();
						if (line.startsWith("Video:"))
						{
							line = line.substring("Video:".length()).trim();
							String[] parts = line.split(",");
//							LOG.v(Arrays.toString(parts));
...
//								LOG.v(part);
								...
								{
									videoFramerate = Float.parseFloat(part.split(" ")[0].trim());
								}
								else if ((matcher = Pattern.compile("(\\d+)x(\\d+)").matcher(part)).find())
								{
									videoWidth = Integer.parseInt(matcher.group(1));
									videoHeight = Integer.parseInt(matcher.group(2));
								}
							}
							videoEncoder = parts[0].trim().split(" ")[0].trim();
						}
						else if (line.startsWith("Audio:"))
						{
							line = line.substring("Audio:".length()).trim();
							String[] parts = line.split(",");
							musicFormat = parts[0].trim().split(" ")[0].trim();
							samplerate = Long.parseLong(parts[1].trim().split(" ")[0].trim());
							musicBitrate = Integer.parseInt(parts[4].trim().split(" ")[0].trim());
						}
					}
				}
			}
		...
		{
			LOG.e("--- 解析视频参数信息出错！ --- 错误信息： " + e.getMessage());
			return null;
		}
	...
	public static VideoMetaInfo getVideoMetaInfo(InputStream inputStream)
	{
		...
		try
		{
			...
			FsUtils.writeBytes(file, inputStream);
		...
		{
			LOG.e("--- 从流中获取视频基本信息出错 --- 错误信息： " + e.getMessage());
			return null;
		}
	}
	...
	public static MusicMetaInfo getMusicMetaInfo(File musicFile)
	{
			...
			if (true)
			{
				...
				for (String line : lines)
				{
					...
					if (line.startsWith("Duration:"))
					{
						...
						long dec = (long) Integer.parseInt(timeParts[1].trim());
						duration = dec + seconds * 1000L + minutes * 60L * 1000L + hours * 60L * 60L * 1000L;
						musicBitrate = Integer.parseInt(parts[2].trim().substring("bitrate:".length()).trim().split(" ")[0].trim());
					}
					else if (line.startsWith("Stream"))
					{
						line = line.substring("Stream #0:0(und):".length()).trim();
		...
		{
			LOG.e("--- 解析音频参数信息出错！ --- 错误信息： " + e.getMessage());
			return null;
		}
	...
	public static MusicMetaInfo getMusicMetaInfo(InputStream inputStream)
	{
		...
		{
			LOG.e("--- 从流中获取音频基本信息出错 --- 错误信息： " + e.getMessage());
			return null;
		}
	}
	...
	public static ImageMetaInfo getImageInfo(InputStream inputStream)
	{
		...
		{
			LOG.e("--- 获取图片的基本信息失败 --- 错误信息： " + e.getMessage());
			return null;
		}
	}
	...
	public static boolean isGivenFormat(File inputFile, String[] givenFormat)
	{
		...
		{
			LOG.e("--- 无法检查文件类型是否满足要求，因为要检查的文件不存在 --- 源文件： " + inputFile);
			return false;
		}
		if (null == givenFormat || givenFormat.length <= 0)
		{
			LOG.e("--- 无法检查文件类型是否满足要求，因为没有指定的文件类型 ---");
			return false;
		}
	...
	private static void createGifImage(String image[], String outputPath, int playTime)
	{
		...
		{
			LOG.e("--- 多张静态图转换成动态GIF图的过程出错 --- 错误信息： " + e.getMessage());
		}
	}
